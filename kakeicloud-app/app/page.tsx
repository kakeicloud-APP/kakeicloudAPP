  ...
  {savedVoucherNo && (
    <div style={{ ...modalOverlay, zIndex: 200 }}>
      <div style={{ background: 'white', padding: '32px', borderRadius: '12px', width: '300px', textAlign: 'center' }}>
        ...
        <button onClick={() => { navigator.clipboard.writeText(savedVoucherNo); alert('copy!') }}
          style={{ width: '100%', padding: '14px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '16px', marginBottom: '8px' }}>copy</button>
        <button onClick={() => setSavedVoucherNo(null)}
          style={{ width: '100%', padding: '12px', background: '#e5e7eb', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>close</button>
      </div>
    </div>
  )}
